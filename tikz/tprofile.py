...
## Python script to automatically generate a TikZ file from a template in order to plot the results of the "task_tprofile" operation of the RecurGreenModal program.
## USAGE : ./tikz/tprofile.py "FILENAME_TPROFILE" "TITLE"
## FILENAME_TPROFILE  = Data file generated by RecurGreenModal containing the rho(T) distribution (Input file, CSV format).
## TITLE              = LaTeX title of the TikZ figure (Input string).
import sys
import os
import compile_tikz

TEMPLATE_TIKZ = "tikz/tprofile.tikz"   ## TikZ template for the "task_distrib" operation of the RecurGreenModal program.
...
def main(args):
    ...
    filename_tikz = os.path.join(os.path.dirname(args[1]), os.path.basename(args[1]).replace(".avg.csv", ".tikz"))
